        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-medium mb-4">Form Edit Program Studi</h3>
                    ...
                    <form action="{{ route('admin.program-studi.update', $programStudi->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('nama') border-red-500 @enderror" id="nama" name="nama" value="{{ old('nama', $programStudi->nama) }}" required>
                            @error('nama')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('jenjang') border-red-500 @enderror" id="jenjang" name="jenjang" value="{{ old('jenjang', $programStudi->jenjang) }}" required>
                            @error('jenjang')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-4">
                            ...
                            <input type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('kode') border-red-500 @enderror" id="kode" name="kode" value="{{ old('kode', $programStudi->kode) }}">
                            @error('kode')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-4">
                            ...
                            <select class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('fakultas_id') border-red-500 @enderror" id="fakultas_id" name="fakultas_id" required>
                                ...
                                @foreach ($fakultas as $fkl)
                                    <option value="{{ $fkl->id }}" {{ old('fakultas_id', $programStudi->fakultas_id) == $fkl->id ? 'selected' : '' }}>{{ $fkl->nama }}</option>
                                @endforeach
                            </select>
                            @error('fakultas_id')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
