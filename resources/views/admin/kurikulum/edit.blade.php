        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-medium mb-4">Form Edit Kurikulum</h3>
                    ...
                    <form action="{{ route('admin.kurikulum.update', $kurikulum->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('nama_kurikulum') border-red-500 @enderror" id="nama_kurikulum" name="nama_kurikulum" value="{{ old('nama_kurikulum', $kurikulum->nama_kurikulum) }}" required>
                        ...
                        <div class="mb-4">
                            ...
                            <select class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('program_studi_id') border-red-500 @enderror" id="program_studi_id" name="program_studi_id" required>
                                ...
                                @foreach ($programStudis as $prodi)
                                    <option value="{{ $prodi->id }}" {{ old('program_studi_id', $kurikulum->program_studi_id) == $prodi->id ? 'selected' : '' }}>{{ $prodi->nama }}</option> {{-- Menggunakan kolom 'nama' --}}
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('tahun_mulai') border-red-500 @enderror" id="tahun_mulai" name="tahun_mulai" value="{{ old('tahun_mulai', $kurikulum->tahun_mulai) }}" required min="1900" max="{{ date('Y') + 5 }}">
                            @error('tahun_mulai')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-4">
                            ...
                            <input type="number" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('tahun_selesai') border-red-500 @enderror" id="tahun_selesai" name="tahun_selesai" value="{{ old('tahun_selesai', $kurikulum->tahun_selesai) }}" min="{{ date('Y') }}" max="{{ date('Y') + 10 }}">
                            @error('tahun_selesai')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-4">
                            ...
                            <textarea class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('deskripsi') border-red-500 @enderror" id="deskripsi" name="deskripsi" rows="3">{{ old('deskripsi', $kurikulum->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-4">
                            <div class="flex items-center">
                                <input type="checkbox" name="is_aktif" id="is_aktif" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('is_aktif', $kurikulum->is_aktif) ? 'checked' : '' }}>
